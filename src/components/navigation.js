...
render(){ return (
	<nav onClick={this.props.toggleNav} className={(this.props.navIsPeeked || this.props.isProjOpen || this.props.navOpen) ? "peeked" : ""} onMouseEnter={(ev)=>{this.props.navPeek(true)}} onMouseLeave={(ev)=>{this.props.navPeek(false, this.props.title)}}>
		...
		<div id="title">
			<Link to="/">
				<h3>
					{this.props.navOpen ? "Malik Dunston" : this.props.title}
				</h3>
			</Link>
		</div>
		{this.props.navOpen ?
